...
def normalize(base_image, root_path):
    base_image = resize(base_image, 600, 400, root_path)
...
def scaling(base_image, w, h, root_path):
    base_image = resize(base_image, 600, 400, root_path)
...
def equalize(base_image, equalizer, root_path):
    base_image = resize(base_image, 600, 400, root_path)   
    matrix = np.array(base_image)
...
def resize(image, max_width, max_height, root_path):
    try:
        ...
        image.save(root_path + '/' + 'static/images/image.png')
